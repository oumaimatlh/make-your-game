...
let pacManElement = null;

function getPacManElement() {
  if (!pacManElement) {
    pacManElement = document.createElement("div");
    pacManElement.id = "pacman-container";
    pacManElement.style.width = "100%";
    pacManElement.style.height = "100%";
    pacManElement.innerHTML = `
...
function pacManRender() {
  ...
  const pacDiv = getPacManElement();
  pacDiv.style.transform = transformStyle;

  if (pacDiv.parentElement !== targetCell) {
    if (pacDiv.parentElement) {
      pacDiv.parentElement.removeChild(pacDiv);
    }
